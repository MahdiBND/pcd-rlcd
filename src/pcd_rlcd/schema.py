    ...
    def __init__(
        ...
        if self.field_type == "boolean":
            ...
            if not choices or len(choices) == 0:
                raise ValueError(
                    f"Field '{name}' of type enum must have choices defined."
                )
            if len(choices) > 255:
                raise ValueError(
                    f"Field '{name}' exceeds maximum cardinality of 255 choices (got {len(choices)})."
                )
    ...
    def to_json_schema_prompt_str(self) -> str:
        ...
        for name, field in self.fields.items():
            if field.field_type == "boolean":
                ...
                choices_str = " | ".join(
                    f'"{c}"' for c in field.choices[:choices_limit]
                )
            ...
            if fdef.field_type == "boolean":
                ...
                for c in fdef.choices:
                    ...
                    cands.append(
                        c_toks[0]
                        if c_toks
                        else tokenizer.encode('"', add_special_tokens=False)[0]
                    )
...
def map_candidate_tokens(
    tokenizer, choices: List[str], is_boolean: bool = False
) -> List[List[int]]:
    """Helper fallback when field definition is not pre-compiled."""
    f = FieldDefinition(
        "tmp",
        "boolean" if is_boolean else "enum",
        "",
        choices if not is_boolean else None,
    )
