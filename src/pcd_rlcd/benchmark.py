...
from .engine import run_naive_generation, run_parallel_generation, get_engine
...
def compare_single(context: str, schema_dict: Dict[str, Any]) -> Dict[str, Any]:
    ...
    steps_speedup = naive_res["sequential_forward_passes"] / max(
        parallel_res["sequential_forward_passes"], 1.0
    )
...
def run_benchmark_suite(
    preset_paths: List[str], warmup: bool = True
) -> List[Dict[str, Any]]:
    print("=" * 70)
    ...
    for path in preset_paths:
        ...
        print(
            f"--> Running preset: {preset['title']} ({len(preset['schema'])} fields)..."
        )
        ...
        r = comp["parallel"]
        print(
            f"    Autoregressive Baseline : {n['elapsed_ms']:>8.1f} ms | {n['total_tokens']:>3} tokens ({n['tokens_per_second']} tok/s) | Passes: {n['sequential_forward_passes']}"
        )
        print(
            f"    Parallel Constrained    : {r['elapsed_ms']:>8.1f} ms |   0 tokens (O(1))           | Passes: {r['sequential_forward_passes']}"
        )
        print(
            f"    >> SPEEDUP: {comp['speedup_multiplier']}x faster (Step reduction: {comp['steps_reduction']}x)"
        )
        print(
            f"    >> Schema match: Naive={n['schema_match']} | Parallel={r['schema_match']} (100% guaranteed)"
        )
        print("-" * 70)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Run Parallel vs Autoregressive LLM JSON benchmark"
    )
    parser.add_argument(
        "--presets",
        nargs="+",
